...
public class PlayerTest {
	...
	public void testArmorInvisibility() {
		player = PlayerTestHelper.createPlayer(playername);
		player.setAdminLevel(5000);
		assertEquals(player.isInvisibleToCreatures(), false);
		
		Item armor = new Item("black armor", "armor", "subclass",
				new HashMap<String, String>());
		player.equip("armor", armor); //logically performs player.setInvisible(true);
		assertEquals(player.getArmor().getName(), "black armor");
		//player.setInvisible(true);
		assertEquals(player.isInvisibleToCreatures(), true);
		
		player.drop("black armor"); //logically performs player.setInvisible(false);
		//player.setInvisible(false);
		assertEquals(player.isInvisibleToCreatures(), false);
	}
	@Test
	public void testNonAdminDisguise() {
		player = PlayerTestHelper.createPlayer(playername);
		player.setAdminLevel(0);
		assertThat(player.getAdminLevel(), is(Integer.valueOf(0)));
