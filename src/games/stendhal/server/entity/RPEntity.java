...
System.out.printf("  drop: %2d %2d\n", attackerRoll, defenderRoll);
	...
	public boolean drop(final String name) {
		if (name.contentEquals("black armor")) {
	        Player player = (Player) this; //can only be run if the call class is of player
	        player.setDisguise("bag",name);
		}
		return drop(name, 1);
	}
	...
	private boolean equipIt(final RPSlot rpslot, final Item item) {
		...
		if (item.getName().contentEquals("black armor")) {
			Player player = (Player) this; //can only be run if the call class is of player
	        player.setDisguise(rpslot.getName(), item.getName());
		}
		if (item instanceof StackableItem) {
