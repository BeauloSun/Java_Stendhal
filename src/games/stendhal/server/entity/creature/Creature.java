...
public class Creature extends NPC {
	...
	/**
	 * check if creature is charmed by player
	 * 
	 * @return the player if true
	 */
	public Player getCharmingPlayer() {
		List<Player> playerList = this.getZone().getPlayers();
		for (Player player : playerList) {
			boolean lequipped = player.isEquippedItemInSlot("lhand", "charming_pipe");
			boolean requipped = player.isEquippedItemInSlot("rhand", "charming_pipe");
			if (lequipped || requipped) {
				return player;
			}
		}
		return null;
	...
	public void logic() {
		healer.heal(this);
		Player player = this.getCharmingPlayer();
		//if creature is attracted by player
		if (player != null && this.getAttackTarget() == player) {
			this.setMovement(player, 0, 0, this.getMovementRange());
			this.setDirection(this.getDirectionToward(player));
			this.applyMovement();
			this.stopAttack();
		}
		else if (!this.getZone().getPlayerAndFriends().isEmpty()) {
