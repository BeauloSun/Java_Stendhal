...
public class StendhalQuestSystem {
	...
	public ArrayList<String> getBankStatementDetail(final Player player, String bankName) {
		ArrayList<String> statement = new ArrayList<String>();
		String eachLine = "";
		PersonalChest a;
		String candidate = "bank_fado";
		switch(bankName) {
		case "Semos Bank":
			candidate = "bank";
			break;
		case "Ados Bank":
			...
			break;
		case "Deniran Bank":
			candidate = "bank_deniran";
			break;
		case "Fado Bank":
			candidate = "bank_fado";
			break;
		case "Nalwor Bank":
			candidate = "bank_nalwor";
			break;
		case "Zaras Bank":
			candidate = "zaras_chest_ados";
			break;
		default:
			candidate = "bank_fado";
		} // bank access value are different from the name that bank teller says, hence to access the actual bank chest, the value needs to be changed.	
		a = new PersonalChest(candidate); 
		a.setAttending(player); 
		for (final RPObject item: a.getBankSlot()) { //recursively go through the whole chest and get any item that is in the slot
			eachLine += item.get("name");
			eachLine += "       ----------------------------       ";
			if (item.get("quantity") == null) {
				eachLine += "1";
			}else {
				eachLine += item.get("quantity");
			}//form a statement string that will be returned and added in the description part of bank statement tab
			statement.add(eachLine);
			eachLine = "";
		}
		return statement;
	}
